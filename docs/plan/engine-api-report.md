# Engine API 探测报告（mdescriptor 0.2.5）

> 日期：2026-08-29（当日 0.2.3 → 0.2.5 升级后重测；0.2.3 初测 2026-08-28，见 git 历史）
> 探测环境：`D:\codex\MD\.venv`（Python 3.12.9）；mdescriptor 0.2.5（cp312 win_amd64 wheel）+ numpy 2.5.2
...
> 升级复核：逐条问题复核结论见 `engine-known-issues.md`；0.2.3→0.2.5 schema diff 要点：
> - 每描述符新增顶层 `execution_engine`（实测全 `"cpp"`）与 `descriptor_version`（全 `"1"`）；per-descriptor `schema_version` 1→2，`get_runtime_info().descriptor_info_schema_version` 1→2
> - DPA4/DPA4C `execution.cooperative_cancel` false→true（现 28/28 为 true）
> - 22/28 `input.mixed_periodicity` false→true（混合周期性批次可用）；**LodeSphericalExpansion 输入能力收窄为仅 `["fully_periodic"]`**
> - 引擎异常新增结构化字段：`code / path / details / message`（如 `code="unsupported_periodicity"`、`path=["input","periodicity"]`），错误映射可按 code 而非异常类型分流；新增公开 `mdescriptor.preload_native()`（Windows 单线程启动期预加载原生二进制）
> - 权威契约文档：引擎仓库 `docs/gui-adaptation-baseline.md`（未随 wheel 发布，已安装环境不可见）
> - 其余（参数类型统计、错误类型、ComputeControl 方法、28 个描述符名单）与 0.2.3 一致
...
- 与设计文档 §8.1 示例相比**更丰富**：多出 `display_name` / `description` / `category` / `capabilities`（list，如 `["cooperative_cancel", "num_threads", "sparse"]`）；0.2.5 再加 `execution_engine`（实际执行内核，全 `"cpp"`）与 `descriptor_version`（结果溯源用，当前全 `"1"`）。
...
实测 3 种变体（0.2.5：`mixed_periodicity` 为能力位——true 表示接受同一批次内混合孤立/周期结构）：
...
- **0.2.3→0.2.5 行为变化**：mixed_periodicity 从整体拒绝变为逐描述符能力位；LodeSphericalExpansion 的 periodicity 由 `["isolated", "fully_periodic"]` 收窄为 `["fully_periodic"]`。
- 含「仅 fully_periodic」的描述符 → ADR-11 兼容性预检（M1 存周期性汇总，M3 禁用不兼容项）有真实触发场景。

### 3.4 Execution Capability

**0.2.3 与 0.2.5 的 28 个描述符均 `devices: ["cpu"]`**（无 CUDA 条目）。v0.1 设备选择固定 CPU，UI 不渲染 GPU 选项；`num_threads` / `cooperative_cancel` 均为 true（逐 descriptor 以 schema 为准，不硬编码；DPA4/DPA4C 的 `cooperative_cancel` 自 0.2.5 起 true）。
...
| 6 | ~~无 per-descriptor version 字段~~ **0.2.5 已修复**：新增 `descriptor_version`，`descriptor_runs.descriptor_version` 已接线记录 | 结果溯源可用真实版本 |
| 7 | ~~DPA4/DPA4C 的 `backend: "numpy"` 标注与实际执行路径不一致~~ **0.2.5 已修复**：新增 `execution_engine`（全 `"cpp"`）承载真实执行路径；`backend` 保留适配器族语义（DPA4/DPA4C 仍 `numpy`）。GUI 信息面板应展示 `execution_engine` 而非仅 `backend` | 信息面板展示 `backend` + `execution_engine` 两字段 |
| 8 | **LodeSphericalExpansion 输入能力收窄**（0.2.3 `["isolated","fully_periodic"]` → 0.2.5 `["fully_periodic"]`），mixed_periodicity 亦为能力位化 | 周期性预检按 schema 动态工作，无需改码；仅升级说明需记录 |
