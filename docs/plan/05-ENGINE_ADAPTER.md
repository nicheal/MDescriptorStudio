...
> 依据：`MDescriptor_GUI_Design.md` §7/§8/§41、`engine-api-report.md`（0.2.5 实测）、ADR-2/ADR-11
...
- 开发与 Release 一律 pin `mdescriptor==0.2.5`（2026-08-29 起；不做 editable 安装；引擎无本地仓库，PyPI 为唯一来源；应用内 UpdateService 走同一 pip 安装路径）。
- 升级流程（固定四步，缺一不可）：
  1. 改 pin 版本号；
...
| GUI 需要 | Adapter 接口 | 引擎调用（0.2.5 实测） |
|---|---|---|
| 启动兼容检查 | `runtime_info()` | `get_runtime_info()` → 五元组版本（0.2.5 起 `descriptor_info_schema_version=2`） |
| Descriptor 列表页 | `list_names()` | `list_descriptors()`（名字 tuple，禁止硬编码） |
| Descriptor 信息面板 / schema 表单 | `schema(name)` | `describe_descriptor(name)`（0.2.5 起 15 键，新增 `execution_engine` / `descriptor_version`；含嵌套 object、input、asset） |
...
|---|---|---|
| `execution.devices` | `devices: list[str]`（0.2.3/0.2.5 全为 `["cpu"]`） | Descriptor 页 Execution 区；无 GPU 时不渲染设备选择 |
| `execution.cooperative_cancel` | `cancelable: bool` | JobService 决定 Cancel 按钮态（否则显示 Cancel unavailable，§23）；0.2.5 起 28/28 均 true，DPA4/DPA4C 实测可中断 |
...
- **启动预热（保留，0.2.3 实测 / 0.2.5 复核）**：0.2.3 中 `create_descriptor` 会懒加载原生扩展模块，若在存在工作线程/stdin 读取线程时触发该 import，会与 import 机制死锁（构建永久阻塞直到 stdin EOF）；**0.2.5 已修复**（`scripts/verify_known_issues.py` 实测构建不再被阻塞的 stdin 卡住）。预热仍保留作为纵深防御，并把首次构建开销付在启动期——main() 在启动任何线程之前，于主线程调用 `adapter.warmup()`（逐个构建全部 28 个描述符并做一次微计算），`backend.ready` 在预热完成后才发出。新增描述符注册路径（如有）不得绕过该预热。
