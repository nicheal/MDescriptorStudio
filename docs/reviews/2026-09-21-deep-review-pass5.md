...
| `<hash7>` | 5-D4 的第一半：删掉 `system.info` 里两个**没人读也没人比较**的版本字段 —— `analysis_api_version`（注释说「载荷形状变了才 bump」，但全仓无一处读它，且它是 mock 手抄版本号里唯一没有门禁钉住的那个）与 `analysis_dependencies`（每次 `system.info` 跑两次 `importlib.metadata.version`，设置面板不显示），连带 `_dependency_version` helper、mock 两处抄本，golden 重生成 | `tests/data/backend-response-keys.json` 少两行；pytest 386 / vitest 208 / Playwright 44 / tsc 干净 |
