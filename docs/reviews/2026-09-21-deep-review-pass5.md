...
### 第 1 批 · 需要一次失效授权 —— 你点头后已落地（`362df30`），`ANALYSIS_ALGORITHM_VERSION` → "studio-analysis-8"
...
**为什么保留两份实现，以及它并不是「互不校验」**：面板要在灵敏度滑杆拖动时立刻出结果，等一次后端作业不现实，
所以浏览器侧的推导是有意的第二实现；而它并不是没人管 —— `frontend/src/pages/trajectoryView.test.ts` 的文件头就写着
「与后端 `_trajectory_threshold` 同一套规则」，逐条钉住 `median + k·1.4826·MAD`、MAD 退化为 0 时改走 `mean + k·σ` 的回退、
以及 `stepStats` 的四分位/偶数长度中位数。也就是说这两份实现的等价性已经被两侧门禁夹住（后端 `test_analysis_engine.py:452/469/472`，
前端 trajectoryView.test.ts），不需要再来一次合并。
**仍然留下的**只有两件小事：`speed` 与 `event_indices` 两个数组后端产出但不在 `registry.ts:140` 的读取清单里（删之前要先确认历史工件的数组名集合不必稳定）；
以及 `event_count`/`events` 这类「后端算了一份、面板自己又算一份」的重复展示，若将来要让面板直接读后端值，那是一次界面决定。
