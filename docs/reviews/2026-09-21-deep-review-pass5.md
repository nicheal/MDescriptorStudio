...
| # | 位置 | 问题 | 已有测量 |
| --- | --- | --- | --- |
| 5-D1 | `docs/plan/02-IPC_PROTOCOL.md:40/49/62/137` | 文档承诺四条实现里不存在的行为：`protocol_version` 不对时「退出 exit 2」（`server.py:180` 回帧后继续服务，`main.py` 只有 0/1，且回帧 `id` 为 null）；`JOB_CANCEL_UNSUPPORTED` 与「前端显示 Cancel unavailable」（码不在 `errors.py`，全仓无此字串）；`dataset.get` 返回 `fingerprint_valid`（服务不写，全仓只有文档这一行提到）；「友好文案由前端按 code 映射」（前端没有映射表，`describeError` 原样渲染后端消息） | 逐行读码 + 全仓 grep |
| 5-D2 | `02-IPC_PROTOCOL.md:58/72/65/§5.1` | 反向漂移：在用且被前端行为依赖的两码（`BUSY` 背压、`DATASET_BUSY` 拒删有活作业的数据集）不在「错误码全集」里（标题写 24、实列 23、`errors.py` 声明 25）；`descriptor.submit` 漏 `num_threads`/`force`/`cache.in_flight`；`dataset.frame` 漏 `ghost_parents`；§5.1 漏 `analysis.fps_quota`/`mantel`/`perturbation_sensitivity` 与 `main.py:129-135` 生成的 17 个 registry 别名 RPC | 脚本比对 errors.py 声明集 ↔ 文档块（in-code-not-doc = `BUSY, DATASET_BUSY`，反向为空）；方法目录 ↔ `ipc.request` 字面量 |
