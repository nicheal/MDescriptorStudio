...
4. **`_analysis_row` 用 `SELECT *`**：`artifact_service.py:153` 在 `get/delete/preview/chunk` 每个热点请求里把可达数十 MB 的 `preview_json` 读出来再 `_json_load` 丢掉，而 `analysis_service.py:44-49` 的注释恰好说明了列表为什么要避开这个 blob；`chunk` 被前端按数组分页循环调用（`Analysis.tsx:462-476`）。 ◑（同类：`dataset_service.py:559-562` 每次打开抽屉为最多 1000 帧重做整趟最近邻，而统计遍历时已算过 `min_distance`，只在 `:509` 留了直方图。—— 这半已在第十三批修掉：扫描把被标记帧的距离随索引一起记下，`STATS_VERSION` 4→5。）
...
7. **可视化树零 memo，一次点选重扫 20k×16**：`Analysis.tsx:1379-1394` 每次渲染新建 props 与内联 `onSelect`；派生计算全在函数体（`analysisVisualizations.tsx:389-400` 16 个 `nums()` 各扫至多 20k、`:295-300` 256²≈65k 单元、`:620-621` 6 个 20k map），`PlotFrame` 无 memo → 每次 job 心跳都重跑并给 Plotly 传新数组引用。 ✎ 已实测，结论是**不做这个重构**（第十三批）：投影面板的 `plot` 早就包在 useMemo 里，点选那次重建是 trace 的 `selectedpoints` 必须改，不是身份抖动。
...
### 第十三批（你定调的四件 + 四-7 的实测，`1cd83a0`…`fdfaf52`）

`462c618` 把被标记帧的最近邻距离随索引一起记进 cached stats（`STATS_VERSION` 4→5：每个数据集下次打开重扫一遍，这是把值放在正确一侧的代价，你已批）。记录时四舍五入到 6 位：融合核与 scipy 参考实现的平方和相加顺序不同，而 `tests/test_native_stats.py` 逐字段比较两个构建的统计结果 —— 正是它抓出了这一点。

`1cd83a0` 让 species 由后端 `deepmd_symbols`（1–118，读文件用的同一张表）解析：前端那份手抄表止于 U:92 且 `.filter(undefined)`，选了 Np/Og 的数据集会静默少掉它们。`cdcc837` 给原子表分页（50/页、可调、表体内部滚动、表头固定），选中点的定位先算页再滚动，纯函数 `atomPageFor` 三条单测钉住。`e8683b5` 把 outliers 的 `k` 收到 LOF/kNN 两个真用它的方法：面板加了控件、提交与缓存 key 同步收窄，isolation forest 与 mahalanobis 不再因别处的 `k` 丢绿点。`7842b63`/`b8161ee` 是 mock 的两件事：数组按 analysis id 归属、已知问题检查器不再自己死在 `readline()` 上。`fdfaf52` 让离线界面说出 shell 日志目录（release 版没有控制台，这是用户唯一能拿到的东西）。
