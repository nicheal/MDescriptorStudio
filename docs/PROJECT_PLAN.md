...
| 项目 .venv | `D:\codex\MD\.venv` 已建立并隔离验证；已装 mdescriptor 0.3.4（2026-09-21 复核，发布构建同源）+ numpy 2.5.2 + pytest 9.1.1，引擎功能基线通过（runtime_info / 28 描述符 / 参数展示元数据 / 全部描述符声明 cpu+cuda） |
| MDescriptor 引擎 | `mdescriptor>=0.3.4`（PyPI，2026-09-21 由 0.3.3 升级；发布构建装最新版，本地实测 0.3.4；CUDA 插件随 wheel 发布），GUI 所需 API 与参数展示元数据完整；实测报告见 `docs/plan/engine-api-report.md` |
...
| 2 | 引擎依赖：PyPI 安装、`mdescriptor>=0.3.4`（发布构建装最新版）；升级走四步流程（05 文档 §2） |
...
| `plan/engine-api-report.md` + `.json` | ✅ | 0.3.4 实测 API 面与参数展示元数据（`scripts/probe_engine.py` 产出） |
