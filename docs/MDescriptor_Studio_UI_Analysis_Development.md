...
| Structural Perturbation Sensitivity | 对**等距抽取的 N 个结构**（`Max structures`，默认 64，可调 1–2048）执行 seeded atomic jitter 或 isotropic strain，逐振幅重算所选 descriptor，统计 mean / median / P95 / max response 曲线；artifact 记录 `sample_count` / `available_structure_count`，发生子采样时给出 warning | response-versus-amplitude curve、per-structure response heatmap、response KPI strip |
