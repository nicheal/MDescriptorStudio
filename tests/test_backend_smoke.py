...
def test_handshake_system_info_and_errors(tmp_path: Path) -> None:
    ...
    try:
        ...
        # A failing assert must not leave an orphaned sidecar holding the
        # data dir; close() is cleanup only here, so it cannot mask the
        # failure that is already being reported.
        bp.close()

    # The exit-code contract still holds, asserted outside the finally.
    assert bp.proc.returncode == 0
