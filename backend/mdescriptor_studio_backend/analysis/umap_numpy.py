...
1. deterministic k-nearest-neighbor search — exact chunked brute force for
   every input size,
...
def _knn_indices_and_distances(
    ...
    The distance matrix is evaluated in row blocks, so the result remains
    exact without materializing the full O(n²) matrix at once. Keeping one
    path for every sample count also makes the neighbour graph auditable.
    """
    n = x.shape[0]
    if metric == "manhattan":
