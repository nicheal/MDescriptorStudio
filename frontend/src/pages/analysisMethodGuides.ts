...
export const ANALYSIS_METHOD_GUIDES: Readonly<Record<string, AnalysisMethodGuide>> = {
  ...
  "overview.feature_variance": {
    ...
    theory: { en: "Feature variance uses population variance (ddof=0) to measure change in each descriptor dimension. Normalized variance is Var(feature) divided by the maximum variance in the current dataset and is only for within-dataset comparison; absolute variance remains sensitive to feature scale and outliers. A feature counts as Constant when its spread is negligible next to its own magnitude (std ≤ 1e-12 × max(|mean|, 1)) or its range is ≤ the constant tolerance - the same rule the preprocessing uses to drop a dimension from every distance, so an absolute range alone is not what decides it. Near-zero means normalized variance < 1e-4, Low variation means 1e-4–<1e-2, and Active means ≥1e-2. These are relative screening thresholds, not universal validity criteria. The detail box plot uses Tukey whiskers (Q1/Q3 ± 1.5 IQR). The histogram counts every finite value while the KDE is drawn over a bounded sample that keeps every detected outlier and is scaled to that sample, so the curve describes the tail, not the bars.", zh: "特征方差使用总体方差（ddof=0）衡量每个描述符维度的变化程度。归一化方差等于特征方差除以当前数据集中的最大方差，仅用于数据集内部比较；绝对方差仍受特征尺度和异常值影响。当某一维的散布相对其自身量级可以忽略（std ≤ 1e-12 × max(|mean|, 1)），或极差 ≤ 常量容差时，记为常量——这与预处理判定「这一维不携带任何信息」用的是同一条规则，因此单看绝对极差并不决定结论。近零方差为归一化方差 < 1e-4，低变化为 1e-4–<1e-2，活跃为 ≥1e-2。这些是相对筛查阈值，不是普适的有效性标准。详情箱线图使用 Tukey 须（Q1/Q3 ± 1.5 IQR）。直方图统计全部有限值，而 KDE 画在一份保留了所有被检出异常值的有界样本上，并按该样本定标，因此曲线描述的是尾部，而不是柱形。" },
